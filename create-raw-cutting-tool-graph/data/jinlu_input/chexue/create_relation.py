...
file = pd.read_excel('data/螺纹加工刀杆关系.xlsx')
data = np.array(file)
data = data.tolist()
sum = 0
for line in tqdm.tqdm(data):
    node_head = graph.nodes.match().where(name='{name}'.format(name=line[0])).first()
    node_tail = graph.nodes.match().where(name='{name}'.format(name=line[2])).first()
